...
#Parte 2: Funções de simulação de voos e hotéis
# app.py - Parte 2

# =========================================================
# Simulação de "APIs" de voos e hotéis (substituíveis por integrações reais)
# =========================================================
def simula_voos(origem: str, destino: str, data_ida: date, data_volta: date):
    """Simula opções de voos de ida e volta com preços e horários."""
...
def simula_hoteis(destino: str, noites: int, cargo: str):
    """Simula opções de hotéis com diária, categoria e custo total."""
...
def buscar_voos_indicative(origem: str, destino: str, data_ida: date, data_volta: date, adultos: int = 1):
    ...
    def parse_indicative(resp_json, trecho_desc, data_str):
        voos = []
        # A estrutura pode variar; usamos campos comuns de preços indicativos
        ...
        for i in itinerarios[:4]:
            preco = None
            cia = "—"
            # Tentativas de extração de preço
            if isinstance(i, dict):
                preco = i.get("price", {}).get("amount") or i.get("pricingOptions", [{}])[0].get("price", {}).get("amount")
                # Agente/cia pode vir como ID; mantemos placeholder
                cia = i.get("pricingOptions", [{}])[0].get("agentIds", ["—"])[0]
            voos.append({
                "trecho": trecho_desc,
                "data": data_str,
                "partida": "00:00",
        ...
        # Fallback mínimo se vazio
        if not voos:
            voos = simula_voos(trecho_desc.split(" → ")[0], trecho_desc.split(" → ")[1], date.fromisoformat(data_str), date.fromisoformat(data_str))[0]
        return voos
